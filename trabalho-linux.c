...
void *FuncaoThreadDistanciaManhattan(void *data)
{
    int id = (int)data, i;
    float resultado = 0;
...
void *FuncaoThreadDistanciaEuclidiana(void *data)
{
    int id = (int)data, i;
    float resultado = 0;
    for (i = id; i < numeroDeLinhasDeTreino; i += numeroDeThreads)
...
int main(int argc, char **argv)
{
    ...
    {
        pthread_t *vetorDeThread = (pthread_t*) malloc(numeroDeThreads * sizeof(pthread_t));
        for (i = 0; i < numeroDeThreads; i++)
        {
            if (algoritmo == 1)
            {
                pthread_create(&vetorDeThread[i], NULL, FuncaoThreadDistanciaEuclidiana, (void*)i);
            }
            else
            {
                pthread_create(&vetorDeThread[i], NULL, FuncaoThreadDistanciaManhattan, (void*)i);
            }
        }
    
        for (i = 0; i < numeroDeThreads; i++)
        {
            pthread_join(vetorDeThread[i], NULL);
        }
        indiceMatrizTeste++;
    }
